...
Public Class frmObjectTypes
    ...
    Private Shared logging As Logging = logging.GetLogger("OSA Config")
    ...
    Public Sub DB_Connection()
        ...
        Try
            CN.Open()
            CN.Close()
            ' logging.AddToLog("Connected to Database: " & Common.DBName & " @ " & Common.DBConnection & ":" & Common.DBPort, "CM15A")
    ...
    Public Sub Write_Wiki()
        Dim CMD As New MySqlCommand, sName As String, sLabel As String, sParam1Label As String, sParam2Label As String
        Dim sParam1Default As String, sParam2Default As String, sHistory As Integer, sPropertyDefault As String
        Dim myReader As MySql.Data.MySqlClient.MySqlDataReader
        frmSQLBox.Show()
        frmSQLBox.txtScript.Text = "{|cellpadding=""5"" cellspacing=""0"" border=""1""" & vbCrLf & "! colspan=""3""| " & txtTypeDescription.Text & " Type States" & vbCrLf
        'frmSQLBox.txtScript.Text &= "| " & txtObjectType.Text & vbCrLf & "| " & txtTypeDescription.Text & vbCrLf & "| " & comTypeOwner.Text & vbCrLf & "| " & comBaseType.Text & vbCrLf
        CMD.Connection = CN
        CMD.CommandText = "SELECT state_name,state_label FROM osae_v_object_type_state WHERE object_type=?ObjectType"
        CMD.Parameters.AddWithValue("?ObjectType", dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Try
            If CN.State = ConnectionState.Closed Then CN.Open()
            myReader = CMD.ExecuteReader()
            frmSQLBox.txtScript.Text &= "|-" & vbCrLf
            frmSQLBox.txtScript.Text &= "| '''Name     " & vbCrLf & "| '''Label      " & vbCrLf & "| '''Comment                    " & vbCrLf
            While myReader.Read
                sName = myReader.Item("state_name")
                sLabel = myReader.Item("state_label")
                frmSQLBox.txtScript.Text &= "|-" & vbCrLf
                frmSQLBox.txtScript.Text &= "| " & sName & vbCrLf & "| " & sLabel & vbCrLf & "| comment" & vbCrLf
            End While
            CN.Close()
            frmSQLBox.txtScript.Text &= "|}" & vbCrLf
        Catch myerror As MySqlException
        ...
        Try
            ...
            frmSQLBox.txtScript.Text &= "{|cellpadding=""5"" cellspacing=""0"" border=""1""" & vbCrLf & "! colspan=""3""| " & txtTypeDescription.Text & " Type Events" & vbCrLf
    ...
    Private Sub butObjectAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butObjectAdd.Click
        OSAEObjectTypeManager.ObjectTypeAdd(txtObjectType.Text, txtTypeDescription.Text, comTypeOwner.Text, comBaseType.Text, Math.Abs(CInt(chkOwner.Checked)), Math.Abs(CInt(chkSystem.Checked)), Math.Abs(CInt(chkContainer.Checked)), Math.Abs(CInt(chkHideRedundant.Checked)))
        Load_Object_Types()
    End Sub
    ...
    Private Sub butObjectDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butObjectDelete.Click
        ...
        If result = Windows.Forms.DialogResult.Yes Then
            OSAEObjectTypeManager.ObjectTypeDelete(dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
            Load_Object_Types()
        End If
    End Sub
    Private Sub butStateAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butStateAdd.Click
        OSAEObjectTypeManager.ObjectTypeStateAdd(txtState.Text, txtStateLabel.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_States()
    End Sub
    Private Sub butStateUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butStateUpdate.Click
        OSAEObjectTypeManager.ObjectTypeStateUpdate(dgvStates("state_name", dgvStates.CurrentCell.RowIndex).Value, txtState.Text, txtStateLabel.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_States()
    End Sub
    Private Sub butStateDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butStateDelete.Click
        OSAEObjectTypeManager.ObjectTypeStateDelete(dgvStates("state_name", dgvStates.CurrentCell.RowIndex).Value, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_States()
    End Sub
    Private Sub butEventAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butEventAdd.Click
        OSAEObjectTypeManager.ObjectTypeEventAdd(txtEvent.Text, txtEventLabel.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_Events()
    End Sub
    Private Sub butEventUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butEventUpdate.Click
        OSAEObjectTypeManager.ObjectTypeEventUpdate(dgvEvents("event_name", dgvEvents.CurrentCell.RowIndex).Value, txtEvent.Text, txtEventLabel.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_Events()
    End Sub
    Private Sub butEventDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butEventDelete.Click
        OSAEObjectTypeManager.ObjectTypeEventDelete(dgvEvents("event_name", dgvEvents.CurrentCell.RowIndex).Value, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_Events()
    End Sub
    Private Sub butMethodAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butMethodAdd.Click
        OSAEObjectTypeManager.ObjectTypeMethodAdd(txtMethod.Text, txtMethodLabel.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value, txtParamLabel1.Text, txtParamLabel2.Text, txtParamDefault1.Text, txtParamDefault2.Text)
        Load_Object_Methods()
    End Sub
    Private Sub butMethodUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butMethodUpdate.Click
        OSAEObjectTypeManager.ObjectTypeMethodUpdate(dgvMethods("method_name", dgvMethods.CurrentCell.RowIndex).Value, txtMethod.Text, txtMethodLabel.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value, txtParamLabel1.Text, txtParamLabel2.Text, txtParamDefault1.Text, txtParamDefault2.Text)
        Load_Object_Methods()
    End Sub
    Private Sub butMethodDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butMethodDelete.Click
        OSAEObjectTypeManager.ObjectTypeMethodDelete(dgvMethods("method_name", dgvMethods.CurrentCell.RowIndex).Value, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_Methods()
    End Sub
    Private Sub butPropertyAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butPropertyAdd.Click
        OSAEObjectTypeManager.ObjectTypePropertyAdd(txtProperty.Text, comPropertyType.Text, txtPropertyDefault.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value, Math.Abs(CInt(chkTrackHistory.Checked)))
    ...
    Private Sub butPropertyUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butPropertyUpdate.Click
        OSAEObjectTypeManager.ObjectTypePropertyUpdate(dgvProperties("property_name", dgvProperties.CurrentCell.RowIndex).Value, txtProperty.Text, comPropertyType.Text, txtPropertyDefault.Text, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value, Math.Abs(CInt(chkTrackHistory.Checked)))
        Load_Object_Properties()
    End Sub
    Private Sub butPropertyDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butPropertyDelete.Click
        OSAEObjectTypeManager.ObjectTypePropertyDelete(dgvProperties("property_name", dgvProperties.CurrentCell.RowIndex).Value, dgvObjectTypes("object_type", dgvObjectTypes.CurrentCell.RowIndex).Value)
        Load_Object_Properties()
    End Sub
    ...
    Private Sub butWiki_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butWiki.Click
        Write_Wiki()
    End Sub
End Class